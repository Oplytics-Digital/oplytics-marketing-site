...
export default function AnimatedDemoShell({
  serviceName,
  accentColor,
  phases,
  children,
}: AnimatedDemoShellProps) {
  ...
  const phaseProgress = (() => {
    ...
    for (let i = 0; i < currentPhase; i++)
      before += phases[i].duration / totalDuration;
    ...
    <div className="relative w-full" style={{ background: "#0a0e1a" }}>
          ...
          {phases.map((phase, i) => (
            <button
              ...
              onClick={() => {
                let t = 0;
                for (let j = 0; j < i; j++)
                  t += phases[j].duration / totalDuration;
                setProgress(t + 0.001);
                setCurrentPhase(i);
              ...
              style={{
                background:
                  currentPhase === i ? `${accentColor}33` : "transparent",
      ...
      <div className="h-0.5 w-full" style={{ background: "#1e2738" }}>
        <div
          ...
          style={{
            width: `${progress * 100}%`,
            background: `linear-gradient(90deg, ${accentColor}, #1DB8CE)`,
          }}
        />
      </div>
