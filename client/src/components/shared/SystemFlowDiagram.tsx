...
function VLink({ label, flip = false }: { label?: string; flip?: boolean }) {
  return (
    <div className="flex flex-col items-center py-1.5">
      {!flip && label && (
        <span className="text-[10px] sm:text-[11px] text-[#596475] tracking-wide mb-1">
          {label}
        </span>
      )}
      <svg
      ...
      {flip && label && (
        <span className="text-[10px] sm:text-[11px] text-[#596475] tracking-wide mt-1">
          {label}
        </span>
      )}
    </div>
...
export default function SystemFlowDiagram() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      >
        An OEE loss or a cross-site OplyticsConnect signal lands on the SQDCP
        board — it links to the relevant Hoshin objective automatically. From
        there it drives an Action, a Safety flag, or a Quality/Certification
        check, and tracks through to verified closure. One shared data model, no
