...
export default function ContactForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (typeof window !== "undefined" && (window as any).umami) {
      (window as any).umami.track("contact_form_submit", {
        context: context || "marketing-site",
      });
    }
    try {
  ...
  if (submitted) {
    return (
      <div
        className={`flex flex-col items-center justify-center ${compact ? "py-8" : "py-16"} text-center`}
      >
  ...
  return (
    <form
      ...
      className="space-y-5"
      data-umami-event="contact_form_start"
      data-umami-event-context={context || "marketing-site"}
    >
      {context && <input type="hidden" name="context" value={context} />}

      <div
        className={
          compact ? "space-y-4" : "grid grid-cols-1 sm:grid-cols-2 gap-5"
        }
      >
        <div>
          <label className="block text-xs font-medium text-[#8890A0] mb-1.5">
            Name *
          </label>
          <input
            type="text"
