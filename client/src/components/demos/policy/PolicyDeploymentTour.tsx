...
type View =
  "dashboard" | "xmatrix" | "bowling" | "actions" | "catchball" | "deployments";
...
function PlaceholderView({
  ...
  return (
    <div
      ...
      style={{ background: "#0e1624", border: "1px solid #1e2738" }}
    >
      <h2 className="text-lg font-black text-white tracking-wide mb-2">
        {title}
      </h2>
      <p className="text-sm max-w-md mx-auto" style={{ color: "#596475" }}>
        {description}
      </p>
    </div>
  );
...
export default function PolicyDeploymentTour() {
  ...
  const moveCursorToView = useCallback((view: View) => {
    ...
    setCursor({
      x: ir.left - cr.left + 26,
      y: ir.top - cr.top + ir.height / 2,
    });
  }, []);
  ...
  const renderView = (view: View) => {
    switch (view) {
      case "dashboard":
        return <PolicyDashboardDemo />;
      case "xmatrix":
        return <XMatrixDemo interactive={done} />;
      case "bowling":
        return <BowlingChartDemo />;
      case "catchball":
        return <CatchballDemo />;
      case "actions":
        return (
          <PlaceholderView
            title="Action Plans"
            description="Track and assign the improvement actions that drive each project. Open the full app to manage the live action register."
          />
        );
      case "deployments":
        return (
          <PlaceholderView
            title="Deployments"
            description="Cascade objectives to individual sites with SQDCP targets. Open the full app to view live site deployments."
          />
        );
    }
  };
  ...
  return (
    ...
    >
      {/* ── Sidebar ── */}
      <aside
        className="shrink-0 w-[210px] flex flex-col"
        style={{ background: "#0b1018", borderRight: "1px solid #1e2738" }}
      >
        {/* Sidebar header */}
        <div
        ...
        <div className="flex-1 py-2 overflow-y-auto">
          ...
          <div className="px-2">
            <div
              className="flex items-center gap-2 h-9 px-3 rounded-md text-xs"
              style={{ color: "#596475" }}
            >
            ...
            {MENU.map(item => {
              ...
              return (
                ...
                >
                  <Icon
                    className="h-4 w-4"
                    style={{ color: isActive ? "#8C34E9" : "#596475" }}
                  />
        ...
        <div className="p-2" style={{ borderTop: "1px solid #1e2738" }}>
          <div
            className="flex items-center gap-2.5 h-9 px-3 rounded-md text-[13px]"
            style={{ color: "#596475" }}
          >
        ...
        >
          <div className="flex items-center gap-2 min-w-0 text-sm">
            ...
            <ChevronRight
              ...
              style={{ color: "#2e3a4e" }}
            />
            <span className="truncate" style={{ color: "#8890a0" }}>
              {labelFor(activeView)}
            </span>
            <span
              className="hidden sm:inline-flex items-center gap-1.5 ml-1 px-2 py-0.5 rounded-md text-[11px] font-medium"
              style={{
                background: "rgba(140,52,233,0.1)",
                color: "#C084FC",
                border: "1px solid rgba(140,52,233,0.2)",
              }}
            >
              <span
                className="w-3.5 h-3.5 rounded-sm flex items-center justify-center text-[8px] font-bold"
                style={{ background: "#8C34E9", color: "#fff" }}
              >
                TG
              </span>
              Testa Group
              ...
              <div className="hidden sm:flex items-center gap-1.5 mr-1">
                {TOUR.map((_, i) => (
                  <span
                    ...
                    style={{
                      background: i <= stepIndex ? "#8C34E9" : "#1e2738",
                    }}
                  />
                ))}
              </div>
            ...
            {!done ? (
              <button
                ...
                style={{
                  ...
                  color: "#C084FC",
                  border: "1px solid rgba(140,52,233,0.25)",
                }}
              >
                {playing ? (
      ...
      {showCursor && (
        ...
        >
          <div className="relative -translate-y-1/2">
            {clicking && (
              <span
                className="absolute -left-1.5 -top-1.5 w-7 h-7 rounded-full animate-ping"
                style={{ background: "rgba(140,52,233,0.4)" }}
              />
            )}
            <MousePointer2
              className="w-5 h-5 drop-shadow-lg"
              style={{ color: "#fff", fill: "#8C34E9" }}
            />
          </div>
        </div>
      )}
